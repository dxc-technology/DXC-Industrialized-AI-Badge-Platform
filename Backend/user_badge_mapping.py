...
def validate_user_id_for_self(user_id):
    if not ObjectId.is_valid(user_id):
        return "User ID is not valid"
    user_details = database.get_users_by_id(ObjectId(user_id))
    user_details_json_dict = json.loads(user_details[0])
    if len(user_details[0]) <= 2:
        return "User ID not Present in User collection DB"
    return True
...
def update_user_badge_status(assertion_id, issuer_id, badge_status_id, comments):
    ...
    return database.update_user_badge_status(assertion_id, issuer_id, badge_status_id, comments)
...
def deleted_user_badge_collection_for_assertion_id_by_user(assertion_id, deleted_by_user_id):
    ...
    if validate_assertion_id(assertion_id) == "Assertion ID not Present in User collection DB":
        return "Assertion ID not Present in User collection DB"
    if validate_user_id_for_self(deleted_by_user_id) == "User ID is not valid":
        return "User ID is not valid"
    if validate_user_id_for_self(deleted_by_user_id) == "User ID not Present in User collection DB":
        return "User ID not Present in User collection DB"
    user_badge_json_val = database.get_all_user_badge_details_by_assertion_id(assertion_id)
    user_badge_json_dict = json.loads(user_badge_json_val[0])
...
def validate_and_update__user_badge_mapping_and_assertions(assertion_id, badge_status_id, work_link, comments,
    ...
    if validate_user_id_for_admin(user_id) == "Requesting user is not an admin":
