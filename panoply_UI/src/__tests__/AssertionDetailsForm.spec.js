...
describe('<AssertionDetailsForm />', () => {
    ...
    describe('On load of Assertion details screen', () => {
        beforeEach(async() => {
            ({ getByTestId } = await render( < AssertionDetailsForm assertionId = '5f83d0c5c091330db1a59123' email ='reviewer2@test.com'/ > ));
        });
        ...
        it('calls the view assertion by Id API ', async() => {
            ...
            UserDetailByEmailResponse.mockResolvedValueOnce(JSON.parse('[{"_id":{"$oid":"6009e2241637bdd807a6aefc"},"email":"reviewer2@test.com","password":"$argon2id$v=19$m=102400,t=2,p=8$gy1AvncFmJcfieeZdb5zQQ$JOZ9sBqKFFukxf9hmMESgg","userType":{"$oid":"5fc5567fcd831cc0c83774b8"},"userStatus":{"$oid":"5f776e5d6289f17659874f27"},"created":{"$date":1611260406290},"modified":{"$date":1611260406290},"firstName":"reviewer2","secondName":"lastname","middleName":"middlename","organizationName":"DXC","user_type_details":[{"_id":{"$oid":"5fc5567fcd831cc0c83774b8"},"type":"reviewer"}],"user_status_details":[{"_id":{"$oid":"5f776e5d6289f17659874f27"},"userStatus":"active"}]}]'));
            expect(UserDetailByEmailResponse).toHaveBeenCalledWith('reviewer2@test.com');
        });
