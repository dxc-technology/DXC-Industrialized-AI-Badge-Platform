...
import ViewProfileForm from '../forms/ViewProfileForm';
...
describe('<UserDetailsForm />', () => {
    ...
    describe('On load of User details screen', () => {
        beforeEach(async() => {
            ({ getByTestId } = await render( < ViewProfileForm userID='5fa9d9eff897b5482159b6a7'/ > ));
        });
        ...
        it('calls the view user by email API ', async() => {
         ...
         it('should populate fields', async() => {
            expect(getByTestId('viewProfile_email').value).toEqual('srikanth123@gmail.com');
            expect(getByTestId('viewProfile_userType').value).toEqual('regular');
            expect(getByTestId('viewProfile_userStatus').value).toEqual('active');
            expect(getByTestId('viewProfile_created').value).toEqual('10-26-2020');
            expect(getByTestId('viewProfile_lastModified').value).toEqual('10-26-2020');
            expect(getByTestId('viewProfile_firstName').value).toEqual('Harry');
            expect(getByTestId('viewProfile_lastName').value).toEqual('Potter');
            expect(getByTestId('viewProfile_middleName').value).toEqual('James');
            expect(getByTestId('viewProfile_organizationName').value).toEqual('DXC Technology');
        });
