...
const ViewAssertionsForm = (props) => {
        ...
        for (var i = 0; i < value.length; i++) {
          filterData.push(
            exportData(
              ...
              value[i].badge_name[0].name,
              formatDate(value[i].issuedOn.$date),
    ...
    if (userType == "5fc5567fcd831cc0c83774b8") {
      var response1 = new Promise((resolve, reject) => {
        ...
        if (value != undefined) {
          ...
          const temp_rows = [];
          for (var i = 0; i < value.length; i++) {
            if (value[i].issuedOn == null)
              temp_rows.push(
                createData(
                  i,
                  value[i].assertion_details[0]._id.$oid,
                  value[i].user_email_address[0].email,
                  value[i].badge_details[0].name,
                  "",
                  value[i].badge_status[0].badgeStatus
                )
              );
  ...
  if (assertionDetailClick == "true") {
    return (
      <div>
        <AssertionDetailsForm
          assertionId={clickedAssertion}
          email={email}
          userType={userType}
          userID={userID}
        />
        ...
        <React.Fragment>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell></TableCell>
                <TableCell data-testid="viewAssertions_badgeName">
                  User
                </TableCell>
                <TableCell>Badge</TableCell>
                <TableCell>Issued On</TableCell>
            ...
            <TableBody>
              {rows.map((row) => (
                <TableRow
                  data-testid={"viewAssertions_RowID" + row.id}
                  key={row.id}
                >
                  <TableCell></TableCell>
                  ...
                  <TableCell align="right">
                    <IconButton
                      data-testid={
                        "viewAssertions_editAssertionButton" + row.id
                      }
                      value={row.mongoId}
                      onClick={handleAssertionDetails}
                    >
                      <EditSharpIcon />
                    </IconButton>
                  </TableCell>
